...
class Boutons_selection:
    ...
    def fun1(self, controller, Xnnn):
        """
        Fonction qui vise a update la frame Info_defaut avec les informations liée au bouton pressé.

        Parameters
        ----------
        controller : tk.frame
            c'est l'app defauthon() en fait
        Xnnn : le nom du défaut en string
            clée dans le Livreionnaire "Livre"
        """
        Info_defaut.label.text.set("TOTO !!!")
    ...
    def bouton_selection_niv3(self, Xnnn, row, column, controller):
        ...
        try:
            ...
            self.boutons[Xnnn] = tk.Button(self, text=Livre[Xnnn][1], relief="raised", bd=10, font=LARGE_FONT,
                                           ...
                                           command=lambda: [Info_defaut.modify(self, Xnnn), controller.show_frame(Info_defaut)])  #, Defauthon.container)])
                                           # command=lambda: [self.fun1(Xnnn), controller.show_frame(Info_defaut)])
...
class Defauthon(tk.Tk):
    ...
    def show_frame(self, cont, *args):
        ...
        """
        if cont == Info_defaut:
            frame = Info_defaut(*args, self)  # sauf que je ne sais pas comment aller chercher le container ! (le *args doit être le container)
            frame.grid(row=0, column=0, sticky="nsew")
            frame.tkraise()
...
class Info_defaut(tk.Frame, Boutons_selection):
    ...
    def __init__(self, parent, controller):
        ...
        self.label = tk.Label(self, text="BUG", font=LARGE_FONT)
        self.label.grid(row=0, column=0, sticky="nsew")
        
    def modify(self, Xnnn):
        """Change le text du label par Xnnn."""
        self.label['text'] = Xnnn
        # self.label.configure(text=Xnnn)
        
        self.label = tk.Label(self, text=Xnnn, font=LARGE_FONT)
        self.label.grid(row=0, column=0, sticky="nsew")
        
        self.label2 = tk.Label(self, text="je n'exsite pas", font=LARGE_FONT)
